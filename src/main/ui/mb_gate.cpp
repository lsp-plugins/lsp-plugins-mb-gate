...
namespace lsp
{
    namespace plugui
    {
        ...
        template <class T>
        T *mb_gate_ui::find_split_widget(const char *fmt, const char *base, size_t id)
        {
            char widget_id[64];
        ...
        mb_gate_ui::split_t *mb_gate_ui::find_split_by_port(ui::IPort *port)
        {
        ...
        mb_gate_ui::mb_gate_ui(const meta::plugin_t *meta): ui::Module(meta)
        {
            fmtStrings      = fmt_strings;
            if (!strcmp(meta->uid, meta::mb_gate_lr.uid))
            {
                fmtStrings      = fmt_strings_lr;
            }
            else if (!strcmp(meta->uid, meta::mb_gate_ms.uid))
            {
                fmtStrings      = fmt_strings_ms;
            }
        }

        mb_gate_ui::~mb_gate_ui()
        {
        }

        status_t mb_gate_ui::slot_split_mouse_in(tk::Widget *sender, void *ptr, void *data)
        {
            // Fetch parameters
            mb_gate_ui *ui = static_cast<mb_gate_ui *>(ptr);
            if (ui == NULL)
        ...
        status_t mb_gate_ui::slot_split_mouse_out(tk::Widget *sender, void *ptr, void *data)
        {
            // Fetch parameters
            mb_gate_ui *ui = static_cast<mb_gate_ui *>(ptr);
            if (ui == NULL)
        ...
        ui::IPort *mb_gate_ui::find_port(const char *fmt, const char *base, size_t id)
        {
            char port_id[32];
        ...
        mb_gate_ui::split_t *mb_gate_ui::find_split_by_widget(tk::Widget *widget)
        {
        ...
        void mb_gate_ui::on_split_mouse_in(split_t *s)
        {
            if (s->wNote != NULL)
            {
        ...
        void mb_gate_ui::on_split_mouse_out()
        {
            for (size_t i=0, n=vSplits.size(); i<n; ++i)
            {
                split_t *d = vSplits.uget(i);
        ...
        void mb_gate_ui::add_splits()
        {
            size_t channel      = 0;
            for (const char **fmt = fmtStrings; *fmt != NULL; ++fmt, ++channel)
            {
                for (size_t port_id=1; port_id<meta::mb_gate_metadata::BANDS_MAX; ++port_id)
                {
                    split_t s;
        ...
        void mb_gate_ui::resort_active_splits()
        {
            vActiveSplits.clear();
        ...
        void mb_gate_ui::update_split_note_text(split_t *s)
        {
            // Get the frequency
            ...
            // Update the note name displayed in the text
            {
                ...
                if (text.ends_with_ascii("m"))
                    lc_string.set("lists.mb_gate.splits.index.mid_id");
                else if (text.ends_with_ascii("s"))
                    lc_string.set("lists.mb_gate.splits.index.side_id");
                else if (text.ends_with_ascii("l"))
                    lc_string.set("lists.mb_gate.splits.index.left_id");
                else if (text.ends_with_ascii("r"))
                    lc_string.set("lists.mb_gate.splits.index.right_id");
                else
                ...
                if (note_full != dspu::NOTE_OUT_OF_RANGE)
                {
                    ...
                    s->wNote->text()->set("lists.mb_gate.notes.full", &params);
                }
                else
                    s->wNote->text()->set("lists.mb_gate.notes.unknown", &params);
            }
        }

        status_t mb_gate_ui::post_init()
        {
        ...
        void mb_gate_ui::notify(ui::IPort *port, size_t flags)
        {
        ...
        void mb_gate_ui::toggle_active_split_fequency(split_t *initiator)
        {
